...
const PhonologicalAwarenessAssistant = () => {
  ...
  const handleSoundIdentification = (sound) => {
    const currentWord = soundIdentificationWords[currentWordIndex];
    if (sound.toLowerCase() === currentWord.sound) {
  ...
  const handleSegmentation = (segments) => {
    const currentWord = segmentationWords[currentWordIndex];
    if (segments === currentWord.length) {
      setScore((prevScore) => prevScore + 1);
        ...
        {activity === 'soundIdentification' && (
          <div>
            ...
            <input
              ...
              onChange={(e) => handleSoundIdentification(e.target.value)}
            />
          </div>
        )}
        ...
        {activity === 'segmentation' && (
          <div>
            ...
            <input
              ...
              className="p-2 border rounded-lg"
              onChange={(e) => handleSegmentation(Number(e.target.value))}
            />
          </div>
        )}
