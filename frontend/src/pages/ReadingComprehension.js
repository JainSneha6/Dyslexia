        ...
        <div className="flex flex-col items-center w-full h-full p-4 rounded-2xl">
          {selectedPassage !== null && (
            <div className="-mt-5 flex flex-col items-center">
              <img src={passages[selectedPassage].image} 
                alt="GIF related to the passage" 
                className="my-2 " 
                style={{ width: '600px', height: '300px' }} />
              <AudioPlayer audio={passages[selectedPassage].voice}/>
            </div>
          )}
                  ...
                  {question.options.map((option) => (
                    <label
                      ...
                      className={`block p-2 border rounded cursor-pointer transition-colors 
                        ${selectedAnswers[index] === option ? (option === question.correctAnswer ? 'bg-green-200' : 'bg-red-200') : 'bg-gray-100'}
                      `}
          ...
          <div className="text-center mt-4">
            <p className="text-lg text-green-600">Score: {score} / {passages[selectedPassage].questions.length}</p>
            <button
